...
(() => {
  ...
  async function home() {
    ...
    try {
      ...
      root.innerHTML=`${currentEstablishmentConfig.demo_scenario?`<div class="fgv-message"><strong>Restaurante de demostración · reservas ficticias</strong><p>Escenario inicial del ${esc(currentEstablishmentConfig.demo_scenario.date)}: pide una mesa para 2 a las 21:00 en Reservas y comprueba las alternativas. Ninguna de estas reservas corresponde a clientes reales.</p></div>`:''}<div class="workspace-kicker">${esc(new Intl.DateTimeFormat('es-ES',{dateStyle:'full',timeZone:currentEstablishment.timezone}).format(new Date()))}</div>
